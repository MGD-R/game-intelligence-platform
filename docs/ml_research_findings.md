...
## Final Defense Readiness Gate

The final readiness layer consolidates the research package into a single artifact check,
metric snapshot and demo sequence. It is intended for the last pre-defense rehearsal: if any
required artifact or key metric is missing, the gate reports `not_ready`.
...
The presentation layer converts the readiness metric snapshot and demo sequence into a
12-slide defense outline, speaker notes and a remaining-step checklist.
...
- `data/artifacts/reports/ml_defense_presentation/ml_defense_presentation_summary.json`
