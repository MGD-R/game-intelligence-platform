...
2. scale `make rawg-index` gradually
3. `make rawg-staging`
...
14. only then targeted `make rawg-details` for selected gaps or problem cases
...
- This avoids the previous broad `wikidata-identity` default, which is too aggressive for a first or quota-sensitive run.
- `rawg-details` is intentionally delayed until after DQ and gap analysis, because it is a more expensive enrichment stage.
