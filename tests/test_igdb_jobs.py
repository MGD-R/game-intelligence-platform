...
from src.ingestion.jobs.load_igdb_search import main as load_igdb_search_main
from src.ingestion.jobs.select_igdb_ids import main as select_igdb_ids_main
from src.ingestion.jobs.select_igdb_search_seeds import main as select_igdb_search_seeds_main
from src.preprocessing.igdb_search_to_candidates import main as igdb_search_to_candidates_main
...
def test_igdb_jobs_support_dry_run() -> None:
    ...
    assert select_igdb_search_seeds_main(["--dry-run", "--limit", "10"]) == 0
    assert load_igdb_games_main(["--dry-run", "--limit", "1"]) == 0
    assert load_igdb_search_main(["--dry-run", "--limit", "1"]) == 0
    assert load_igdb_reference_main(["--dry-run"]) == 0
    assert igdb_to_staging_main(["--dry-run"]) == 0
    assert igdb_search_to_candidates_main(["--dry-run"]) == 0
