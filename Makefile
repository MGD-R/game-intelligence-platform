...
.PHONY: build build-dev up up-dev up-mlops up-notebook up-admin down logs ps shell db-shell \
	...
	er-baseline er-merge-strategy-comparison er-graph-analysis ml-research-defense code-graph code-graph-watch code-graph-mcp export-data-pack import-data-pack restore-from-files export-raw-cache data-pack-check \
...
ml-research-defense:
	$(COMPOSE) up -d postgres
	$(WORKER_RUN) python -m src.entity_resolution.build_research_defense_artifacts
